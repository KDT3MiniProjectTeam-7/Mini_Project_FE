...
1. 추천 상품 
  - 회원 정보 호출을 통해 이름과 인사말 출력 
  - 설문을 통해 회원 정보에 담긴 태그를 활용 상품 정보를 호출
  - 상품 정보를 카테고리 별로 보여주며 보기 편하도록 토글 기능 추가
  - 만일 설문을 통한 태그가 있지 않은 회원이라면 설문 유도
</details>
...
1. 설문 form
  - 상품 정보들에 담긴 태그를 참고하여 설문 작성
  - 사용자가 설문하기 편하도록 객관식 질문에 radio 활용
...
1. 카테고리 별 적절한 내용 반영
   ...
   - 여러 사이트의 사례 참고
      - 뱅크샐러드/뱅킹사이트 등등 
   - 온라인 신청 버튼
      - 신청 기능은 없지만, 실제 신청페이지로 접근가능하게 해놓음

2. 장바구니 추가 가능
   - 상세 페이지에서 장바구니 추가가능
      - 우측 상단의 하트 버튼으로 추가/제거 가능
      - 색상변경으로 사용자가 알아볼수있게함
...
- 👍 협업 관련
    ...
    - 백엔드와 첫 협업 경험이어서 의논할 게 무엇이고, 어떤 역할을 해야하는지 파악하는게 어려웠음.
