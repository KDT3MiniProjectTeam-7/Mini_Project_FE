...
const Agreement = (props: any) => {
  ...
  useEffect(() => {
    if (checkOne === checkTwo) {
      setCheckAll(checkOne);
      const checkbox: HTMLInputElement = document.getElementById('check_all') as HTMLInputElement;
      checkbox.checked = checkOne;
    } else {
      setCheckAll(false);
      const checkbox: HTMLInputElement = document.getElementById('check_all') as HTMLInputElement;
  ...
  const handleChecked = (event: any) => {
    if (event.target.id === 'check_all') {
      ...
      if (event.target.id === 'check_1') {
        setCheckOne((current) => !current);
      } else if (event.target.id === 'check_2') {
        setCheckTwo((current) => !current);
      }
    }
  };
  ...
  return (
    <Container>
      <form>
        <section>
          <div className="checkContainer allTitle">
            <input type="checkbox" name="item" id="check_all" onClick={(event) => handleChecked(event)} />
            <label className="check_all" htmlFor="check_all">
              전체 동의
            </label>
          </div>
          <div className="checkContainer">
            <input type="checkbox" name="item" id="check_1" onClick={(event) => handleChecked(event)} />
            <label htmlFor="check_1">
              이용약관 동의<span> (필수)</span>
            </label>
          </div>
          <pre>{`
제1조(목적) 이 약관은 파이낸스세븐(전자상거래 사업자)가 운영하는 파이낸스세븐앱(이하 “앱”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 파이낸스세븐앱과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.
 
※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」

제2조(정의)

① “앱”이란 파이낸스세븐이 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
④ “앱”이 약관을 개정할 경우에는 적용일자 및 개정사유를 명시하여 현행약관과 함께 몰의 초기화면에 그 적용일자 7일 이전부터 적용일자 전일까지 공지합니다. 다만, 이용자에게 불리하게 약관내용을 변경하는 경우에는 최소한 30일 이상의 사전 유예기간을 두고 공지합니다.  이 경우 "몰“은 개정 전 내용과 개정 후 내용을 명확하게 비교하여 이용자가 알기 쉽도록 표시합니다. 
...
② “앱”이 제3자에게 구매자 개인정보를 제공·위탁할 필요가 있는 경우 실제 구매신청 시 구매자의 동의를 받아야 하며, 회원가입 시 미리 포괄적으로 동의를 받지 않습니다. 이 때 “앱”은 제공되는 개인정보 항목, 제공받는 자, 제공받는 자의 개인정보 이용 목적 및 보유‧이용 기간 등을 구매자에게 명시하여야 합니다. 다만 「정보통신망이용촉진 및 정보보호 등에 관한 법률」 제25조 제1항에 의한 개인정보 취급위탁의 경우 등 관련 법령에 달리 정함이 있는 경우에는 그에 따릅니다.
...
⑤ “앱”이 제3항과 제4항에 의해 이용자의 동의를 받아야 하는 경우에는 개인정보관리 책임자의 신원(소속, 성명 및 전화번호, 기타 연락처), 정보의 수집목적 및 이용목적, 제3자에 대한 정보제공 관련사항(제공받은자, 제공목적 및 제공할 정보의 내용) 등 「정보통신망 이용촉진 및 정보보호 등에 관한 법률」 제22조제2항이 규정한 사항을 미리 명시하거나 고지해야 하며 이용자는 언제든지 이 동의를 철회할 수 있습니다.
...
⑨ “앱”은 개인정보의 수집·이용·제공에 관한 동의란을 미리 선택한 것으로 설정해두지 않습니다. 또한 개인정보의 수집·이용·제공에 관한 이용자의 동의거절시 제한되는 서비스를 구체적으로 명시하고, 필수수집항목이 아닌 개인정보의 수집·이용·제공에 관한 이용자의 동의 거절을 이유로 회원가입 등 서비스 제공을 제한하거나 거절하지 않습니다.
...
본 약관은 2023년 2월 24일부터 적용한다.
`}</pre>
        </section>
        <section>
          <div className="checkContainer">
            <input type="checkbox" name="item" id="check_2" onClick={(event) => handleChecked(event)} />
...
현 개인정보처리방침은 2022년 2월 24일에 제정되었으며, 정부 및 회사의 정책 또는 보안기술의 변경에 따라 내용의 추가, 삭제 및 수정이 있을 경우에는 개정 최소 7일 전부터 ‘공지사항’란을 통해 고지하며, 본 정책은 시행 일자에 시행됩니다.
1) 공고일자 : 2022년 2월 24일
2) 시행일자 : 2022년 2월 24일
`}</pre>{' '}
        </section>
      </form>
      <BottomContainer isAllChecked={checkAll}>
        <button className="nextButton" onClick={clickNext}>
          동의하고 계속하기
        </button>
      </BottomContainer>
    </Container>
...
const BottomContainer = styled.div<AllCheckedData>`
  width: 100%;
  position: absolute;
  bottom: 0;
  left: 0;
